...
import { Checkbox, Group, Modal, Table, Title, UnstyledButton, Text, Button, TextInput } from "@mantine/core"
  ...
  return (
    <Group direction="column" grow>
        ...
        <thead>
          <tr>
            <th>
              <Checkbox
                onChange={(e) => {
                  setSelected((v) => v.length === ((users.filter((member) => member.id === user?.id).length * -1) + users.length) ? [] : users.filter((member) => member.id !== user?.id).map((member) => member.id))
                }}
                checked={selected.length === ((users.filter((member) => member.id === user?.id).length * -1) + users.length)}
                indeterminate={selected.length > 0 && selected.length !== ((users.filter((member) => member.id === user?.id).length * -1) + users.length)}
              />
            </th>
            {columns.map((field) => {
              return (<th key={field.field}>
                {field.headerName}
              </th>)
            })}
          </tr>
      ...
      <Modal
        ...
        title={`Create User`}
      >
        <form onSubmit={createMemberForm.onSubmit((user) => {
